package com.migros.tracking.common.exception.handler;

import com.migros.tracking.common.exception.ErrorDetailDto;
import com.migros.tracking.common.exception.ErrorDetailDtoHolder;
import com.migros.tracking.common.exception.MessageType;
import com.migros.tracking.common.util.JsonUtil;
...
public class BaseExceptionHandler {
    ...
    public ResponseEntity<ErrorDetailDtoHolder> handleMethodArgumentNotValidException(MethodArgumentNotValidException ex) {
        ...
        Class<?> targetClass = ex.getBindingResult().getTarget() != null ? ex.getBindingResult().getTarget().getClass() : null;
        ...
        ErrorDetailDtoHolder holder = ErrorDetailDtoHolder.builder()
                                                          .message(messageSource.getMessage("error.invalid.request", null,
    ...
    private ErrorDetailDto getDtoFromFieldError(String field, String message) {
        return ErrorDetailDto.builder().type(MessageType.VALIDATION.name()).code(field).message(message).build();

    }

    private ErrorDetailDto getDtoFromObjectError(String objectName, String message) {
        return ErrorDetailDto.builder().type(MessageType.BUSINESS.name()).code(objectName).message(message).build();
    }
