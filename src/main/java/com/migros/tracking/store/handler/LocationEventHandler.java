package com.migros.tracking.store.handler;

import com.migros.tracking.common.service.DistanceCalculationService;
...
import static com.migros.tracking.common.config.async.AsyncConfiguration.DEFAULT_THREAD_EXECUTOR_NAME;
...
public class LocationEventHandler {
    ...
    public void handle(LocationRequest event) {
        ...
        log.info("Courier with id: {}, entranced to {}, distance to store is : {}m, last total trip is: {}m", event.courierId(),
                 nearestStore.getStoreName(), nearestStore.getDistanceToStore(), totalDistance);
    }
    ...
    private List<CourierLocationDto> getCourierLocationDtos(LocationRequest event, Instant lastEntranceTime) {
        ...
        return Stream.iterate(firstPage, Objects::nonNull, page -> {
            ...
            return nextPageNumber <= page.getTotalPages() ?
                    ...
                    null;
        }).flatMap(Streamable::get).sorted(Comparator.comparing(CourierLocationDto::time)).toList();
    }
